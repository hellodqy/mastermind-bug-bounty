...
- 安全头缺失、版本/指纹、Self-XSS、TLS 配置警告、单独开放重定向、缺少频率限制、报错/堆栈/Java 类名、单独账号枚举、未实测成功的爆破、只有 AccessKeyId、无服务端绕过的硬编码加密密钥、无可复现 PoC 的发现都禁止报告。
- 如果这些低价值信号串成真实越权、注入、RCE、数据窃取或账号接管，只报告最终链路结果，不报告中间环节。
