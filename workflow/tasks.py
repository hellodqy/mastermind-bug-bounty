...
""", "findings/_candidate_findings.json", True)]),
    ...
    Task("t4_validate_leads", "test", 6, "Turn credential and exposure leads into validated candidates",
         [TaskStep(1, "ai", "Investigate leads and preserve only demonstrated impact",
          "Read _candidate_findings.json, _source_leaks.txt, _exposure_probe.json, _external_assets.json, _endpoint_params.json, and the Phase 1 attack-surface queue. Write findings/_validated_candidates.json and drop unsuccessful leads entirely. Every retained item must include src_eligibility with boundary_crossed=true, reproducible=true, outcome_type, affected_asset, and evidence. Allowed outcomes are unauthorized_sensitive_data, unauthorized_sensitive_action, account_takeover, privilege_escalation, code_execution, financial_loss, meaningful_cross_user_impact, or service_compromise. API credentials additionally require credential_validation. Internal/Swagger/OpenAPI/Druid paths additionally require exposure_validation. PageSpy/VConsole/sourcemaps additionally require artifact_validation proving sensitive data or action. URL tokens additionally require token_transport_validation proving third-party exposure, replay, and sensitive impact. Never retain a version, path, domain, tool, source map, authenticated endpoint, or design pattern by itself.",
